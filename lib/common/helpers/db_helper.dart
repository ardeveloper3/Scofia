...
class DBHelper {
  ...
  static Future<sql.Database> db()async{
    return sql.openDatabase("scofia",version: 1,
  ...
  static Future<int> creatItem(Task task)async{
  ...
  static Future<int> creatUser(int isVerified)async{
    final db = await DBHelper.db();
    final data = {
      'id':1,
  ...
  static Future<int> updateItem(int id, String title, String desc,
    ...
    final results = await db.update('todos', data,
    where: "id = ?",
      whereArgs: [id]
    );
    return results ;
